...
extern Txt Kinput;
extern Txt KonData;
extern Txt Ktranscribe__file;
extern Txt Kvosk;
extern Txt Kwindow;
extern unsigned char D_obj_p_3_test_00transcribe_20file[];
void obj_p_3_test_00transcribe_20file( Asm4d_globals *glob, tProcessGlobals *ctx, int32_t inNbExplicitParam, int32_t inNbParam, PCV inParams[], CV *outResult)
{
	CallChain c(ctx,D_obj_p_3_test_00transcribe_20file);
	if (!ctx->doingAbort && c.f.fLine==0) {
		...
		}
		{
			...
			if (g->Call(ctx,(PCV[]){nullptr,Kvosk.cv(),Ktranscribe__file.cv(),t5.cv()},3,1389)) goto _0;
		}
_2:
_0:
_1:
